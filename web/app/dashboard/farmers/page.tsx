...
export default function FarmersPage() {
  ...
  const [form, setForm] = useState<NewFarmerForm>(emptyForm);
    ...
    <div>
      ...
      {showForm && (
        <div className="mb-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Add New Farmer</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">No farmers found. Add your first farmer above.</div>
      ) : (
        <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-gray-600">Primary Crops</th>
                    <th className="text-center px-4 py-3 font-semibold text-gray-600">Total Deals</th>
                    <th className="text-center px-4 py-3 font-semibold text-gray-600">Quality Rating</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {farmers.map((f) => (
                    <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-600">{f.primary_crops}</td>
                      <td className="px-4 py-3 text-center text-gray-700">{f.total_deals}</td>
                      <td className="px-4 py-3 text-center">{renderStars(f.quality_rating)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex items-center justify-between mt-4">
            <span className="text-sm text-gray-500">Page {page + 1} · {farmers.length} results</span>
